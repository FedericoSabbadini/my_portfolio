/* =========================================================================
   ...
   Left = region overview · Centre = the neural hierarchy (canvas) · Right =
   detail. EVERY region renders in the same graph so nothing feels bolted on;
   Work is laid out as a chronological chain rather than a radial star.
   ========================================================================= */
import { TreeGraph } from './tree-graph.js';
import { renderDetail, renderEmpty } from './node-panel.js';
import { getChildren, getPath, getRegionStats, getRegionLayout } from '../data/build-tree.js';
...
export class TreeView {
  ...
  open(regionId, nodeId) {
    ...
    this.fg.setTree(this.tree, regionId, this.region.accent, getRegionLayout(regionId));
  ...
  _renderLeft() {
    ...
    const branchLabel = this.regionId === 'work' ? 'Steps' : (this.regionId === 'contacts' ? 'Channels' : 'Branches');

    // About gets a real profile intro (bio + headline stats) in the overview
    let intro = `<p class="dom-desc">${esc(d.desc)}</p>`;
    let statsBlock = `<div class="dom-stats">${stat(s.items, 'Items')}${stat(children.length, branchLabel)}</div>`;
    if (this.regionId === 'about' && d.meta) {
      const bio = (d.meta.bio || []).map((p) => `<p>${esc(p)}</p>`).join('');
      intro = `<div class="dom-bio">${bio}</div>`;
      statsBlock = `<div class="dom-stats dom-stats--about">${(d.meta.stats || []).map((x) =>
        stat(esc(x.number), `${esc(x.label)}${x.sublabel ? ` ${esc(x.sublabel)}` : ''}`)).join('')}</div>`;
    }

    const nav = `<div class="dom-section-title">Explore</div><div class="dom-nav">${children.map((n) => `
